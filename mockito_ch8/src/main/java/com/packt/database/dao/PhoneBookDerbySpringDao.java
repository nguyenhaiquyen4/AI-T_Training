...
public class PhoneBookDerbySpringDao implements PhoneBookDao {
    ...
    public boolean create(PhoneEntry entry) {
        int rowCount = jdbcTemplate.update("insert into PhoneBook values (?,?,?)",
                                           new Object[] {
                                                   entry.getPhoneNumber(), entry.getFirstName(),
                                                   entry.getLastName()
                                           });
        // Why 1 ?
        // the jdbcTemplate.update actually not invoked, how can it return 1 ?
        return rowCount == 1;
    }
    ...
    public List<PhoneEntry> searchByNumber(String number) {
        return jdbcTemplate.query("SELECT * FROM PhoneBook where num=?", new Object[] { number },
                                  new PhoneEntryRowMapper());
    }

    @Override
    public List<PhoneEntry> searchByFirstName(String firstName) {
        return jdbcTemplate.query("SELECT * FROM PhoneBook where fname=?", new Object[] { firstName },
                                  new PhoneEntryRowMapper());
    }

    @Override
    public List<PhoneEntry> searchByLastName(String lastName) {
        return jdbcTemplate.query("SELECT * FROM PhoneBook where lname=?", new Object[] { lastName },
                                  new PhoneEntryRowMapper());
    }
